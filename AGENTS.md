...
- **Avoid allocations in hot paths**: `exclude.rs` uses `Cow` for path normalization (only allocates on Windows when backslashes present). `filedetect.rs` uses `String::with_capacity` + `push` instead of `format!`.
